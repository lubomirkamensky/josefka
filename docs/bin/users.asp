...
if output == None:
	...
	sqlgate.cn2 = ""
	requests = TimeTree2.users.Requests(utilitygate=utilitygate, xmlgate=xmlgate, sqlgate=sqlgate, formgate=formgate, session=Session.SessionID, user="", appname="users", cachepath=cachepath, project="josefka")
	output = requests.tools[ID].output(ID)
	utilitygate.writeCache(cachepath,output)

utilitygate.getTimeControl(start,"Celkem: ")
utilitygate.recordSystemUsage ("template","4",start)
%>
