...
cachepath="C:/projects/josefka/machine/cache/webtools/"
output = utilitygate.readCache(cachepath)
if output == None:
	sqlgate.cn = sqlgate.open("dsn=webtools;uid=root;pwd=securemarc")
	sqlgate.cn2 = ""
	requests = TimeTree2.webtools.Requests(utilitygate=utilitygate, xmlgate=xmlgate, sqlgate=sqlgate, formgate=formgate, session=Session.SessionID, user="aib", appname="josefka", cachepath=cachepath, project="josefka")
	output = requests.tools[ID].output(ID)
	utilitygate.writeCache(cachepath,output)

utilitygate.getTimeControl(start,"Celkem: ")
utilitygate.recordSystemUsage ("template","2",start)
