...
cachepath="C:/projects/josefka/machine/cache/shared/"
output = utilitygate.readCache(cachepath)
if output == None:
	sqlgate.cn = sqlgate.open("dsn=shared;uid=root;pwd=securemarc")
	sqlgate.cn2 = ""
	requests = TimeTree2.shared.Requests(utilitygate=utilitygate, xmlgate=xmlgate, sqlgate=sqlgate, formgate=formgate, session=Session.SessionID, user="aib", appname="josefka", cachepath=cachepath, project="josefka")
	output = requests.tools[ID].output(ID)
	utilitygate.writeCache(cachepath,output)

utilitygate.getTimeControl(start,"Celkem: ")
utilitygate.recordSystemUsage ("template","1",start)
%>
